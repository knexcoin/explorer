...
const KnexCore = {
    config: {
        ...
        tierColors: {
            1: '#00ff00', // Mega — bright green
            2: '#33cc33', // Large — medium green
    ...
    renderTopology() {
        ...
        const TILT = -0.4; // ~23deg to show both hemispheres
    ...
    _drawGreatCircleArc(ctx, nodeA, nodeB, cx, cy, R, yaw, tilt) {
        ...
        ctx.strokeStyle = `rgba(51,255,51,${baseAlpha.toFixed(3)})`;
    ...
    _drawParticles(ctx, nodes, cx, cy, R, yaw, tilt) {
        ...
        for (let p = 0; p < 3; p++) {
            ...
            ctx.fillStyle = `rgba(51,255,51,${alpha.toFixed(2)})`;
            ctx.fill();
        }
    },
    ...
    renderNerdStats() {
        ...
        const enforcedColor = ci.consensus_enforced ? '#00e676' : '#33ff33';
